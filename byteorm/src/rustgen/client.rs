    ...
    quote! {
        ...
        impl #accessor_struct {
                ...
                FU: FnMut(#update_builder, T) -> #update_builder,
            {
                ...
                let mut all_values: Vec<String> = Vec::with_capacity(records.len());
                let mut all_params: Vec<Box<dyn tokio_postgres::types::ToSql + Sync + Send>> =
                    vec![];
                let mut param_idx = 1;

                for record in records {
                    let create_builder =
                        create(#create_builder::new(self.pool.clone()), record.clone());
                    ...
                    if let Some(expected) = &insert_columns_template {
                        if expected != &insert_columns {
                            return Err(
                                "upsert_many requires the same create columns for every record"
                                    .into(),
                            );
                        }
                    } else {
                    ...
                    for col in update_set_fragments {
                        if !seen_update_columns.insert(col) {
                            return Err(
                                format!("Duplicate update field in upsert_many: {}", col).into(),
                            );
                        }
                    ...
                    if let Some(expected) = &update_columns_template {
                        if expected != &update_columns {
                            return Err(
                                "upsert_many requires the same update columns for every record"
                                    .into(),
                            );
                        }
                    } else {
